...
export default async (req: Request, res: AppResponse) => {
  ...
  if (!filters)
    filters = {
      ...
      filter: { field: 'created_at', order: 'ASC' },
    };
  else {
  ...
  try {
    ...
    const [items, count] = await Items.findAndCount({
      skip,
      take,
      order: { [field]: order },
    });
    ...
    // return data
    return res.status(200).json({ success: true, data: { hasMore, records: items, total: count } });
  } catch (err) {
